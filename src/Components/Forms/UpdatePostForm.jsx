...
const UpdatePostForm = ({ post, handleCloseForm, fetchPosts, filter }) => {
  ...
  const handleForm = async (e) => {
    ...
    const id = post?.id
    if (!id) {
      console.log("No se encontró el ID de la publicación")
      return
    }
      ...
      <div className="flex justify-center items-center h-screen">
        <form onSubmit={handleForm} className="bg-[#191D23] w-[600px] mx-8 flex flex-col gap-2 p-8 rounded">
          ...
          <div className="pt-8 self-end flex gap-4">
            ...
            <button type="submit" className="text-sm cursor-pointer bg-white hover:bg-[#eeeeee] text-[#262C36] py-2 px-6 rounded transition">Actualizar publicación</button>
          </div>
        </form>
      </div>
    </section>
